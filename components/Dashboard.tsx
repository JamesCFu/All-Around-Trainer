  ...
  return (
    <div className="max-w-5xl mx-auto">
      <header className="mb-10 flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          <h2 className="text-4xl font-black text-slate-900 tracking-tight">Scholar's Hub</h2>
          <p className="text-slate-500 mt-2 font-medium">Elevating your potential for the MCVSD admissions.</p>
        </div>
        <div className="bg-white p-5 rounded-[2rem] shadow-sm border border-slate-100 flex items-center space-x-4 min-w-[240px]">
      ...
      <div className="relative overflow-hidden bg-gradient-to-br from-indigo-700 to-indigo-900 rounded-[3rem] p-10 mb-12 shadow-2xl text-white">
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-8">
          <div className="max-w-xl">
            ...
            <h3 className="text-3xl font-black mb-4 tracking-tight">Elite Simulation Engine</h3>
            <p className="text-indigo-100 mb-8 leading-relaxed text-lg">Experience the full rigor of the high-difficulty MCVSD exam. Comprehensive logic, literature, and math diagnostics.</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
        <div className="bg-white p-8 rounded-3xl shadow-sm border border-slate-100 flex items-center space-x-5">
          ...
          <div>
            ...
            <div className="text-3xl font-black text-slate-900 tracking-tight">{stats.questionsAnswered || 0}</div>
          </div>
        </div>
        <div className="bg-white p-8 rounded-3xl shadow-sm border border-slate-100 flex items-center space-x-5">
        ...
        <div className="bg-white p-8 rounded-3xl shadow-sm border border-slate-100 flex items-center space-x-5">
          ...
          <div>
            ...
            <div className="text-3xl font-black text-slate-900 tracking-tight">{stats.xp || 0}</div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {Object.values(Category).filter(c => c !== Category.MOCK).map((cat) => (
          <div key={cat} className="bg-white p-8 rounded-[2rem] shadow-sm border border-slate-100 hover:border-indigo-300 transition-all hover:shadow-xl group cursor-pointer" onClick={() => onStartPractice(cat)}>
            <div className="flex justify-between items-center mb-6">
              <h4 className="font-black text-xl text-slate-900 tracking-tight">{cat}</h4>
              <span className="text-[10px] font-black bg-indigo-50 text-indigo-700 px-3 py-1 rounded-full border border-indigo-100">{stats.categoryScores[cat] || 0}% Acc</span>
            </div>
            <div className="w-full bg-slate-100 h-2.5 rounded-full overflow-hidden mb-8 shadow-inner">
              <div className="h-full bg-indigo-600 transition-all duration-700" style={{ width: `${stats.categoryScores[cat] || 0}%` }}></div>
            </div>
